...
def get_timezone():
    ...
    if tmz and tmz in pytz.all_timezones:
        return tmz
    try:
        tmz = g.user['timezone']
        if tmz in pytz.all_timezones:
            return tmz
    except Exception:
        tmz = None
...
def before_request():
    ...
    if usr:
        ...
        return render_template('6-index.html', logger=logger)
    else:
        return render_template('6-index.html', logger=_('not_logged_in'))
